...
OUT_DIR="${1:-${ROOT_DIR}/examples/experiment/short_video_negative_feedback/output_metric_probe_balanced_n10}"

echo "[metric-probe-balanced-n10] output dir: ${OUT_DIR}"
mkdir -p "${OUT_DIR}"
...
python "${ROOT_DIR}/examples/experiment/short_video_negative_feedback/run_experiment.py" \
  ...
  --recsys-profile balanced_sensitive \
...
print("[metric-probe-balanced-n10] key metrics:")
for metric in [
...
echo "[metric-probe-balanced-n10] done."
echo "[metric-probe-balanced-n10] outputs:"
