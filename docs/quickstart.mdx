...
<AccordionGroup>
  ...
  <Accordion icon="github" title="Option 2: Clone the repository (uv)">
    ```bash
    ...
    # Now run tutorial scripts normally, for example:
    # python examples/quick_start.py
    ```
    On Windows, install uv with `winget install --id=astral-sh.uv -e`, then
    reopen the terminal before running the commands above. Activate the
    environment with `.venv\Scripts\activate.bat` in Command Prompt or
